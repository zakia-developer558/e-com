...
const getVariantDetails = (product: Product, colorId: number | null) => {
  ...
  return {
    ...
    // Update the images array to prioritize color-specific images
...
export default function ProductGrid({ limit, products: overrideProducts }: { limit?: number, products?: Product[] } = {}) {
  const router = useRouter();
  const searchParams = useSearchParams();
  
  // Track current image index for each product using product_id as key
  ...
  // Get filtered images for selected color
  ...
  const handleColorSelect = (colorId: number, productId: string) => {
    ...
    // Reset image index for this product
    setCurrentImageIndices(prev => ({
      ...prev,
      [productId]: 0
    }));
      ...
      <div className="px-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {shownProducts.map((product: Product) => {
            const variant = getVariantDetails(product, selectedColors[product.product_id]);
            const filteredImages = getFilteredImages(product);
            
            return (
              <div key={product.product_id} className="group cursor-pointer" onClick={() => router.push(`/products/${product.product_id}`)}>
                  ...
                  {filteredImages.length > 1 && (
                    <>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          prevImage(product.product_id, filteredImages.length);
                        }}
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          nextImage(product.product_id, filteredImages.length);
                        }}
                    ...
                    {product.colors.map((colorId) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleColorSelect(color.id, product.product_id)
                          }}
                          className={`w-5 h-5 rounded-full border ${selectedColors[product.product_id] === color.id ? 'ring-2 ring-offset-2 ring-gray-400' : ''}`}
